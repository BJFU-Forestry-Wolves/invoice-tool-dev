...
后续 P1 发现 PaddleOCR 3.7 的 JSON 结果外层为 `{"res": ...}`。P0 原冒烟只验证了推理调用未报错，没有验证内部文字块非空，因此其“识别成功”不能作为文本适配正确性的证据。适配器现已解包 `res` 并增加非空文字块测试；修正后的 P1 全量缓存验证为 214 个唯一内容页、14,867 个文字块、0 个空页。P0 冻结包需在下一次打包验收时用修正后的适配器重建。
